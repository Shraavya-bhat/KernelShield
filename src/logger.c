...
{
    switch (type) {
    case KS_EVENT_EXEC:      return "exec";
    case KS_EVENT_EXIT:      return "exit";
    case KS_EVENT_NETWORK:   return "network";
    case KS_EVENT_PRIVILEGE: return "privilege";
    case KS_EVENT_FILE:      return "file";
    case KS_EVENT_ALERT:     return "alert";
    default:                 return "unknown";
    }
}

static const char *file_operation_name(uint32_t operation)
{
    switch (operation) {
    case KS_FILE_OPEN:    return "open";
    case KS_FILE_WRITE:   return "write";
    case KS_FILE_CREATE:  return "create";
    case KS_FILE_RENAME:  return "rename";
    case KS_FILE_DELETE:  return "delete";
    case KS_FILE_EXECUTE: return "execute";
    default:              return "unknown";
    }
}
...
static void write_common_fields(FILE *fp,
...
void ks_logger_event(const struct ks_event *event)
{
    ...
    if (event->type == KS_EVENT_EXIT) {
        ...
        char dst_ip[INET_ADDRSTRLEN] = "unknown";
    ...
    else if (event->type == KS_EVENT_FILE) {
        ...
        fprintf(log_file,
                ...
                "\"size\":%llu,"
                "\"path\":\"",
                file_operation_name(event->file_operation),
                event->file_operation,
                event->file_mode,
                (unsigned long long)event->file_size);

        json_escape(log_file, event->file_path);
...
void ks_logger_alert(const char *severity,
